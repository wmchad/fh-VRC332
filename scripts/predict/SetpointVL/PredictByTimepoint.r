...
outputFolder <- "~/Projects/VRC332/Data/PredictSetpointVL"
for ( tp in 0:8 ) {
  cat("    Starting Timepoint", tp, "\n")
  tpData <- GetTimepointData(vlData, tp, predSummary, TRUE, "LogSetpointVL")
  tpData$x <- ReplaceMissingWithMean(tpData$x)
  outdir <- file.path(outputFolder, paste("tp", tp, "-", tpLevels[tp+1], sep=""))
  dir.create(outdir, showWarnings=FALSE, recursive=TRUE)
  predResults <- RunRandomPartitionPredictions(
                   tpData, pTrain=pTrain, nRand=nRand, method="glmnet", ctrl=ctrl,
                   outdir=outdir,
                   rmseFile=paste("tp", tp, "-", tpLevels[tp+1], "-CompRmse.txt", sep=""),
                   fitsFile=paste("tp", tp, "-", tpLevels[tp+1], "-Fits.rdata", sep=""),
                   verbose=TRUE, progressEvery=10)

  bestPredData <- GetBestPredictorsData(vlData, predResults, nRand/2, TRUE, "LogSetpointVL")
  bestPredData$x <- ReplaceMissingWithMean(bestPredData$x)
  predResults <- RunRandomPartitionPredictions(
                   bestPredData, pTrain=pTrain, nRand=nRand, method="glmnet", ctrl=ctrl,
                   outdir=outdir,
                   rmseFile=paste("tp", tp, "-", tpLevels[tp+1], "-BestPred-CompRmse.txt", sep=""),
                   fitsFile=paste("tp", tp, "-", tpLevels[tp+1], "-BestPred-Fits.rdata", sep=""),
                   verbose=TRUE, progressEvery=10)
}
